...
ngReason: 'サービス提供元へTCP接続できませんでした。ファイアウォールやセキュリティ機器でブロックされている可能性があります。',
action: 'この通信ができないとサービスを利用できません。ネットワーク管理者またはご利用のネットワーク機器のベンダーへ「キヅクモサービスのドメインに対するTCP 443番ポートの許可」をご相談ください。キヅクモサービスのドメインに関しては担当営業へお問い合わせください。'
},
{
label: '通信ポート確認',
...
action: '一部ポートが閉じていると映像配信や接続に支障が出る可能性があります。ネットワーク管理者またはご利用のネットワーク機器のベンダーへ「キヅクモサービスで利用するポートの許可」をご相談ください。キヅクモサービスのポートに関しては担当営業へお問い合わせください。'
},
{
label: '接続方式',
description: '接続方式を候補および最適な候補の選定',
keyword: '【 接続方式候補 】',
tooltip: 'WebRTCのcandidate情報から得られた候補を表示します',
detail: '通信に使用されたcandidateの種類（host/srflx/relay）をもとに、どの方式で通信が確立されたかを表示します。',
ngReason: 'WebRTC接続が確立していないため、候補情報が取得できませんでした。',
action: 'WebRTC接続が成功している場合のみ候補情報が表示されます。接続失敗時はネットワーク環境やブラウザ設定をご確認ください。'
},
{
label: 'WebRTC接続成功',
